...
//
// The file is the one that sat in `packages/voidbase/src/server/plugins/polar.ts`, with its relative imports written as
// the published names they already resolved to: `../errors` is `/sdk`, `../interfaces`, `../types` and `./manifest`
...
import { apiPrefix, base64, constantTimeEqual, depsWith, ensureCustomer, fromBase64, hmacSha256, isoDate, knob, obj, paymentsPlugin, str, upsert, webhookPathOf, webhookResult, REFERENCE_KEY, TOLERANCE_SECONDS, } from "@voidbase-cloud/voidbase/plugins/payments-shared";
...
export const accessToken = (env) => knob(env, KEY_VAR);
export const webhookSecret = (env) => knob(env, WEBHOOK_SECRET_VAR);
export const isSandbox = (env) => /^(1|true|yes)$/i.test(knob(env, SANDBOX_VAR));
export const apiBase = (env) => (isSandbox(env) ? POLAR_SANDBOX_API : POLAR_API);
// ---- the wire -------------------------------------------------------------------------------------
async function call(fetchFn, base, token, method, path, body) {
 ...
 */
export function webhookKeys(secret) {
  const keys = [secret];
  const encoded = secret.startsWith("whsec_") ? secret.slice(6) : secret;
  try {
    keys.unshift(fromBase64(encoded));
  }
...
export async function signPayload(secret, id, timestamp, payload, scheme = "standard") {
 ...
 */
export async function verifySignature(payload, headers, secret, o = {}) {
  if (!secret)
    return { ok: false, reason: `no ${WEBHOOK_SECRET_VAR} to verify against` };
  if (!headers.id || !headers.timestamp || !headers.signature)
    return { ok: false, reason: "webhook-id, webhook-timestamp and webhook-signature are all needed" };
  const t = Number(headers.timestamp);
  if (!Number.isFinite(t) || t <= 0)
    return { ok: false, reason: "webhook-timestamp is not unix seconds" };
  const sigs = headers.signature.split(/\s+/).filter((s) => s.startsWith("v1,")).map((s) => s.slice(3));
  if (!sigs.length)
    return { ok: false, reason: "no v1 signature in webhook-signature" };
...
const firstPrice = (sub) => str(obj(sub.prices?.[0]).id);
const customerOf = (o) => ({ id: str(o.customer_id || obj(o.customer).id), email: str(obj(o.customer).email), user: str(obj(o.customer).external_id) });
/** a Polar order's status as a payments row's: `paid` says succeeded; a refund says refunded; anything else is pending */
const orderStatus = (o) => (o.status === "refunded" || o.status === "partially_refunded" ? "refunded" : o.paid === true || o.status === "paid" ? "succeeded" : o.status === "void" ? "canceled" : "pending");
...
const checkCharge = (o) => {
  if (o.amounts?.length)
    throw badRequest(`Polar charges the products it sells and has no line for an amount, so a checkout with ${o.amounts.map((a) => a.name).join(", ")} cannot be charged in full through Polar and is refused rather than charged without them`);
  if (!o.reference)
    return;
  if (o.items.length !== 1)
    throw badRequest(`a Polar checkout for an order takes one item: Polar charges for the one product the buyer picks of those it lists, so ${o.items.length} items cannot be charged in full through Polar and are refused rather than charged for one of them`);
  const item = o.items[0];
  if (item.quantity > 1)
    throw badRequest(`a Polar checkout for an order takes a quantity of 1: Polar charges a product once whatever quantity it is sent, so ${item.quantity} of "${item.price}" cannot be charged in full through Polar and are refused rather than charged for one`);
};
/** Polar over its dependencies: the knobs, the calls, the signature, the events */
export function polarProvider(deps) {
  const mustToken = (env) => {
    const token = accessToken(env);
    if (!token)
      throw new ApiError(503, `Polar is not configured on this instance: set ${KEY_VAR} (a secret) and, for the webhook, ${WEBHOOK_SECRET_VAR}`);
    return token;
  };
  const api = (env) => {
    const token = mustToken(env), base = apiBase(env);
    return (method, path, body) => call(deps.fetch, base, token, method, path, body);
  };
  return {
    ...
    async createCustomer(env, auth) {
      ...
      if (email) {
        ...
        const existing = obj(found.items?.[0]);
        if (existing.id)
    ...
    verify: (payload, headers, secret, now) => verifySignature(payload, { id: headers.get("webhook-id"), timestamp: headers.get("webhook-timestamp"), signature: headers.get("webhook-signature") }, secret, { now }),
    applyEvent,
  };
}
/** the plugin over its dependencies: the shipped one uses fetch, D1 and the clock; tests replace all three */
export function polarWith(overrides = {}) {
