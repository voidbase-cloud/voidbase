...
// pull requests and their comments (the previews plugin): POST /repos/<o>/<r>/pulls makes one in the mock ({ head, title?,
// state?, merged? }), PATCH /repos/<o>/<r>/pulls/<n> changes its state; the rest is GitHub's shape
interface GhPull { number: number; state: "open" | "closed"; merged_at: string | null; html_url: string; title: string; head: { ref: string }; updated_at: string }
const ghPulls = new Map<number, GhPull>(); const ghComments = new Map<number, { id: number; issue: number; body: string }>(); let ghPullNo = 0;
...
seedGh(); const zones = [{ id: "zone123", name: "example.com" }]; const sendingDomains = new Set<string>(); /* Email Sending: domains onboarded on zone123 */ const domains = new Map<string, { id: string; hostname: string; service: string; zone_id: string; environment: string; cert_id: string }>(); const uploadedHashes = new Set<string>(); const pendingSessions = new Map<string, Set<string>>(); const calls: string[] = [];
// the certificate a custom domain gets (the domain's cert_id names a pack on its zone): pending on the first read of the
// zone's packs, active from the second, so a deploy's wait is exercised without waiting; the zone's redirect rules
// (Rulesets API entrypoint of the http_request_dynamic_redirect phase) as the deploy and the domains plugin set them
...
Bun.serve({ port, hostname: "127.0.0.1", maxRequestBodySize: 200 * 1024 * 1024, async fetch(req) {
  ...
  // ---- GitHub (unauthenticated in the mock): the repository, releases and asset uploads, pull requests and comments
  if (p === `/repos/${GH_REPO.owner.login}/${GH_REPO.name}`) return Response.json(GH_REPO);
  { const m = p.match(/^\/repos\/([^/]+)\/[^/]+\/pulls$/); if (m) {
      if (req.method === "POST") { const b = (await req.json()) as { head: string; title?: string; state?: "open" | "closed"; merged?: boolean }; const number = ++ghPullNo; const pr: GhPull = { number, state: b.state ?? "open", merged_at: b.merged ? new Date().toISOString() : null, html_url: `https://github.com/${m[1]}/x/pull/${number}`, title: b.title ?? b.head, head: { ref: b.head }, updated_at: new Date().toISOString() }; ghPulls.set(number, pr); return Response.json(pr, { status: 201 }); }
      const head = url.searchParams.get("head"); const state = url.searchParams.get("state") ?? "open"; const ref = head?.includes(":") ? head.slice(head.indexOf(":") + 1) : head;
      return Response.json([...ghPulls.values()].filter((x) => (!ref || x.head.ref === ref) && (state === "all" || x.state === state)).reverse()); } }
  { const m = p.match(/^\/repos\/[^/]+\/[^/]+\/pulls\/(\d+)$/); if (m) { const pr = ghPulls.get(Number(m[1])); if (!pr) return Response.json({ message: "Not Found" }, { status: 404 }); if (req.method === "PATCH") { const b = (await req.json()) as { state?: "open" | "closed"; merged?: boolean }; if (b.state) pr.state = b.state; if (b.merged) { pr.state = "closed"; pr.merged_at = new Date().toISOString(); } } return Response.json(pr); } }
  { const m = p.match(/^\/repos\/[^/]+\/[^/]+\/issues\/(\d+)\/comments$/); if (m) { const issue = Number(m[1]); if (!ghPulls.has(issue)) return Response.json({ message: "Not Found" }, { status: 404 });
      if (req.method === "POST") { const b = (await req.json()) as { body: string }; const c = { id: ++ghIds, issue, body: b.body }; ghComments.set(c.id, c); return Response.json(c, { status: 201 }); }
      return Response.json([...ghComments.values()].filter((c) => c.issue === issue)); } }
  { const m = p.match(/^\/repos\/[^/]+\/[^/]+\/issues\/comments\/(\d+)$/); if (m && req.method === "PATCH") { const c = ghComments.get(Number(m[1])); if (!c) return Response.json({ message: "Not Found" }, { status: 404 }); const b = (await req.json()) as { body?: string }; if (typeof b.body === "string") c.body = b.body; return Response.json(c); } }
  ...
  // Email Sending: the zone's sending domains (POST onboards one, GET lists them with `enabled`)
  { const m = p.match(/^\/zones\/([^/]+)\/email\/sending\/subdomains$/); if (m) { if (!zones.some((z) => z.id === m[1])) return err(404, 1000, "zone not found"); if (req.method === "POST") { const b = (await req.json()) as { name: string }; sendingDomains.add(b.name); return ok({ name: b.name, enabled: true }); } return ok([...sendingDomains].map((name) => ({ name, enabled: true }))); } }
  ...
  if (p === `${A}/workers/domains` && req.method === "PUT") { const b = (await req.json()) as { hostname: string; service: string; zone_id: string; environment?: string }; if (!zones.some((z) => z.id === b.zone_id)) return err(404, 100116, "zone not found"); if (!scripts.has(b.service)) return err(404, 10007, "script not found"); const id = crypto.randomUUID().replace(/-/g, ""); const cert_id = crypto.randomUUID(); const d = { id, hostname: b.hostname, service: b.service, zone_id: b.zone_id, environment: b.environment ?? "production", cert_id }; domains.set(id, d); (certPacks.get(b.zone_id) ?? certPacks.set(b.zone_id, []).get(b.zone_id)!).push({ id: cert_id, type: "advanced", hosts: [b.hostname], reads: 0 }); return ok(d); }
  { const m = p.match(new RegExp(`^${A}/workers/domains/([^/]+)$`)); if (m) { const d = domains.get(m[1]!); if (!d) return err(404, 100117, "domain not found"); if (req.method === "GET") return ok(d); if (req.method === "DELETE") { domains.delete(m[1]!); return new Response(null, { status: 204 }); } } }
  { const m = p.match(/^\/zones\/([^/]+)\/ssl\/certificate_packs$/); if (m && req.method === "GET") { if (!zones.some((z) => z.id === m[1])) return err(404, 1000, "zone not found"); return ok((certPacks.get(m[1]!) ?? []).map((c) => { c.reads++; return { id: c.id, type: c.type, hosts: c.hosts, status: c.reads >= 2 ? "active" : "pending_validation", certificate_authority: "google", validation_method: "txt" }; })); } }
  { const m = p.match(/^\/zones\/([^/]+)\/rulesets\/phases\/http_request_dynamic_redirect\/entrypoint$/); if (m) { if (!zones.some((z) => z.id === m[1])) return err(404, 1000, "zone not found"); const zone = m[1]!;
      if (req.method === "GET") { const rules = rulesets.get(zone); return rules ? ok({ id: `rs-${zone}`, phase: "http_request_dynamic_redirect", rules }) : err(404, 20211, "could not find ruleset"); }
      if (req.method === "PUT") { const b = (await req.json()) as { rules?: Record<string, unknown>[] }; if (!Array.isArray(b.rules)) return err(400, 20003, "rules required"); const rules = b.rules.map((r) => ({ ...r, id: (r.id as string | undefined) ?? crypto.randomUUID().replace(/-/g, "") })); rulesets.set(zone, rules); return ok({ id: `rs-${zone}`, phase: "http_request_dynamic_redirect", rules }); } } }
