...
The repository is a Bun workspace and this package is `packages/voidbase`; the commands below are run from the
repository root, which is where the tooling and the hoisted `node_modules/.bin` live.
