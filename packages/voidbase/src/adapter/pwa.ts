...
export interface WorkerConfig { version: string; precache: string[]; start: string;
  /** paths the worker never touches beside /api/ and /_/: the `panel` option's path, which is not this app */
  never?: string[] }
...
export function generateServiceWorker(cfg: WorkerConfig): string {
  ...
  const never = ["/api/", "/_/", ...(cfg.never ?? [])].map((p) => JSON.stringify(p)).join(", ");
...
const NEVER = [${never}]; // voidbase's API and panel: always the network
const PRECACHE = [
${precache}
];
...
export async function writePwa(root: string, publicDir: string, opts: PwaOptions, never: string[] = []): Promise<PwaResult> {
  ...
  writeFileSync(join(publicDir, WORKER_FILE), generateServiceWorker({ version, precache, start, never }));
