...
`packages/voidbase/scripts/schema-sql.ts`, which the repository keeps and the package does not ship, kept current
by a unit test) and applied once by the object's constructor, recorded in
