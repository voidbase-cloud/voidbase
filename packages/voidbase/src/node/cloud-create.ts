...
export async function createOnCloudflare(o: { api: CfApi; account: string; name: string; email: string; log?: (line: string) => void }): Promise<Created> {
  ...
  const r = await provisionInstance(o.api, { account: o.account, name: o.name, release: releaseFromDir(built.dir), superuser: { email: o.email, password }, applyDoMigrations: true, log: (l) => log(`  ${l}`) });
