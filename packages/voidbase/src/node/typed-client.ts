...
export function generateTypes(doc: OpenApiDocument, opts: GenerateOptions): string {
  ...
  const header = [
    ...
    `//   ${opts.regenerate}`,
    `// The types come from the instance's own OpenAPI description (GET /api/openapi.json, as ${scopeOf(doc)} sees it), which`,
    "// the server builds from the same collection definitions and rules it enforces, so this file and /api/docs cannot",
    "// disagree, and it holds only what that caller may reach.",
  ];
...
export interface FetchOptions {
  ...
  url: string;
  /** any token: the document describes what its holder may reach. Without one, email and password sign in through
   * _superusers/auth-with-password, and with neither the document is a guest's */
  token?: string;
  email?: string;
  password?: string;
  fetch?: typeof fetch;
...
export function openSession(opts: FetchOptions): DocumentSession {
  ...
  return {
    async document(): Promise<OpenApiDocument> {
      // nothing to sign in with: the guest's document, which is every bit as much the instance's answer
      if (!token && canSignIn) token = await signIn();
      let r = await get();
...
/** the document as the caller sees it: every collection a superuser, only what its rules let anyone else reach */
export async function fetchDocument(opts: FetchOptions): Promise<OpenApiDocument> { return openSession(opts).document(); }

/** who the document was written for, as a phrase: the server scopes it to the caller (anonymous, user, superuser) */
export function scopeOf(doc: OpenApiDocument): string {
  const scope = doc.info?.["x-voidbase"]?.scope;
  return scope === "anonymous" ? "a guest" : scope ? `a ${scope}` : "an unknown caller";
}

/** the document must be an OpenAPI document of a voidbase instance; whose scope it is, is the caller's business */
