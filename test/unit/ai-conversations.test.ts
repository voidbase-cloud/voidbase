...
import { invalidateCollections, listCollections, type Collection } from "../../src/server/collections/model";
...
describe("on an instance: the collections, the rows through the records service, the cascade", () => {
  ...
  async function instance() {
    ...
    for (const file of readdirSync(`${ROOT}/db/migrations`).filter((x) => x.endsWith(".sql")).sort()) for (const s of readFileSync(`${ROOT}/db/migrations/${file}`, "utf8").split("--> statement-breakpoint")) if (s.trim()) sqlite.exec(s);
    const db = d1(sqlite);
