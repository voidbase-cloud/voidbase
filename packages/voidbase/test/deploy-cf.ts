...
const run = (args: string[], env: Record<string, string> = {}) => { const p = Bun.spawnSync(["bun", BIN, ...args], { cwd: dir, env: { ...process.env, CLOUDFLARE_API_BASE: MOCK, VOIDBASE_DEPLOY_CF_API_KEY: "", CLOUDFLARE_API_TOKEN: "", VOIDBASE_SUPERUSER_EMAIL: "", VOIDBASE_SUPERUSER_PASSWORD: "", PB_SUPERUSER_EMAIL: "", PB_SUPERUSER_PASSWORD: "", VOIDBASE_HOOKS_DIR: "", VOIDBASE_MIGRATIONS_DIR: "", VOIDBASE_MAIL_DOMAIN: "", VOIDBASE_AI: "", VOIDBASE_SEO_PNG: "", VOIDBASE_OBSERVABILITY: "", VOIDBASE_OBSERVABILITY_SAMPLE: "", VOIDBASE_OBSERVABILITY_ACCOUNT_ID: "", VOIDBASE_OBSERVABILITY_TOKEN: "", VOIDBASE_DOMAINS: "", VOIDBASE_DEPLOY_DOMAIN: "", ...env } }); return { code: p.exitCode, out: new TextDecoder().decode(p.stdout) + new TextDecoder().decode(p.stderr) }; };
try {
  ...
  // VOIDBASE_AUTH_COOKIE makes a cookie authenticate a request, and a browser attaches a cookie on its own: the
  // deploy refuses the knob unless one of the two CSRF protections is on (a dry run reports what it would refuse)
  const openCookie = run(["deploy", "--dry-run", "--public-dir", "../sk/build"], { VOIDBASE_DEPLOY_CF_API_KEY: "cf-test-token", VOIDBASE_AUTH_COOKIE: "1" });
  check("VOIDBASE_AUTH_COOKIE with neither CSRF protection is refused by the deploy, naming both ways to fix it", /VOIDBASE_AUTH_COOKIE was refused/.test(openCookie.out) && /VOIDBASE_CORS_ORIGINS/.test(openCookie.out) && /VOIDBASE_CSRF=double-submit/.test(openCookie.out), openCookie.out.slice(-400));
  const guardedCookie = run(["deploy", "--dry-run", "--public-dir", "../sk/build"], { VOIDBASE_DEPLOY_CF_API_KEY: "cf-test-token", VOIDBASE_AUTH_COOKIE: "1", VOIDBASE_CSRF: "double-submit" });
  check("with a protection on it is not refused, and neither is a deploy that never asked for the cookie", !/VOIDBASE_AUTH_COOKIE was refused/.test(guardedCookie.out) && !/VOIDBASE_AUTH_COOKIE was refused/.test(first.out), guardedCookie.out.slice(-300));
  ...
  // Workers Observability (the observability plugin's deploy half, docs/plugins.md "Seeing what the instance is
  // doing"): on by default, its rate shared with the plugin's own sampling, and off by one knob
  const jsonc = (name: string) => { const f = `${PKG}/.cloud/${name}/wrangler.jsonc`; return existsSync(f) ? JSON.parse(readFileSync(f, "utf8").replace(/^\/\/.*\n/, "")) as Record<string, unknown> : {}; };
  const envOf = (name: string) => { const f = `${PKG}/.cloud/${name}/.env`; return existsSync(f) ? readFileSync(f, "utf8") : ""; };
  const defaultObs = (JSON.parse(cfg.replace(/^\/\/.*\n/, "")) as { observability?: unknown }).observability;
  check("observability is on by default: the Worker's logs are kept at head_sampling_rate 1, and no knob is baked", JSON.stringify(defaultObs) === JSON.stringify({ enabled: true, head_sampling_rate: 1 }) && !envOf("shopdemo-backend").includes("VOIDBASE_OBSERVABILITY") && /observability: the Worker's logs and invocation logs are kept \(head_sampling_rate 1\)/.test(first.out), `${JSON.stringify(defaultObs)} :: ${first.out.split("\n").filter((l) => /observability/.test(l)).join(" | ").slice(0, 300)}`);
  const sampled = run(["deploy", "--dry-run", "--name", "obs-api"], { VOIDBASE_DEPLOY_CF_API_KEY: "cf-test-token", VOIDBASE_OBSERVABILITY_SAMPLE: "0.1", VOIDBASE_OBSERVABILITY_ACCOUNT_ID: "acc999", VOIDBASE_OBSERVABILITY_TOKEN: "a-real-secret" });
  check("VOIDBASE_OBSERVABILITY_SAMPLE lowers head_sampling_rate and is baked as the plugin's rate; the account id is baked and the token never is", sampled.code === 0 && JSON.stringify(jsonc("obs-api").observability) === JSON.stringify({ enabled: true, head_sampling_rate: 0.1 }) && envOf("obs-api").includes("VOIDBASE_OBSERVABILITY_SAMPLE=0.1") && envOf("obs-api").includes("VOIDBASE_OBSERVABILITY_ACCOUNT_ID=acc999") && !envOf("obs-api").includes("a-real-secret") && !envOf("obs-api").includes("VOIDBASE_OBSERVABILITY_TOKEN"), `${JSON.stringify(jsonc("obs-api").observability)} :: ${JSON.stringify(envOf("obs-api"))}`);
  const obsOff = run(["deploy", "--dry-run", "--name", "obs-off-api"], { VOIDBASE_DEPLOY_CF_API_KEY: "cf-test-token", VOIDBASE_OBSERVABILITY: "0" });
  check("VOIDBASE_OBSERVABILITY=0: no observability in the config, and the knob is baked so the plugin stops sampling too", obsOff.code === 0 && jsonc("obs-off-api").observability === undefined && envOf("obs-off-api").includes("VOIDBASE_OBSERVABILITY=0") && /observability off \(VOIDBASE_OBSERVABILITY=0\)/.test(obsOff.out), `${JSON.stringify(jsonc("obs-off-api").observability)} :: ${obsOff.out.slice(-300)}`);
  const badSample = run(["deploy", "--dry-run", "--name", "obs-bad-api"], { VOIDBASE_DEPLOY_CF_API_KEY: "cf-test-token", VOIDBASE_OBSERVABILITY_SAMPLE: "2" });
  check("a sampling rate that is not a rate stops the deploy with the knob's name", badSample.code === 1 && /VOIDBASE_OBSERVABILITY_SAMPLE=2 is not a number between 0 and 1/.test(badSample.out), badSample.out.slice(-300));
