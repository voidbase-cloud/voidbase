...
`VOIDBASE_HOOKS_DIR` / `VOIDBASE_MIGRATIONS_DIR`, or run `scripts/dev.sh start 5180` from the repository root
(`scripts/` is the repository's own tooling and is not part of the package) and edit `pb_hooks/`:
