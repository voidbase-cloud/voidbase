...
export async function voidbase(opts: ServeOptions = {}) {
  ...
  const start = async (): Promise<VoidbaseServer> => {
    ...
    if (!process.env.VOIDBASE_PROJECT_BAKED) {
      ...
      // the core a version runs on: this process, recorded as it runs, and a newer one fetched into pb_data/cores when an
      // update asks for it (src/node/core.ts)
      const core = await import("./core");
      const emb = await embedded();
      const running = emb?.version ?? (JSON.parse(readFileSync(`${PKG}/package.json`, "utf8")) as { version: string }).version;
      const self = { version: running, argv: emb ? [process.execPath] : [process.execPath, resolve(PKG, "bin/voidbase.ts")] };
      ...
      // a second server on the same pb_data (sync up reading the instance) leaves the running one's record alone
      if (core.servesInstance(dir, process.pid)) core.writeServeInfo(dir, { pid: process.pid, http: `http://127.0.0.1:${port}`, version: running, requests: true });
      process.on("SIGUSR2", () => { if (!core.servesInstance(dir, process.pid)) return; const request = core.takeRequest(dir); if (!request) return; try { if ("update" in request) rebuilder.queue(`update voidbase ${running} -> ${request.update}`, { core: request.update }); else rebuilder.rollback(request.rollback); } catch (err) { console.error(`voidbase: ${err instanceof Error ? err.message : err}`); } });
    }
