...
try {
  ...
  await step("request-otp sends the code", async (s) => { await sinkClear(); const r = await s.api("POST", "/api/collections/users/request-otp", { email: USER.email }); const [m] = await sinkWait(1); s.st.otpId = String(r.json?.otpId ?? ""); s.st.otp = otpFrom(m); return { status: r.status, idLen: s.st.otpId.length, otpLen: s.st.otp.length, mail: normalize(m) }; });
  ...
  await step("mfa: otp with mfaId completes", async (s) => { await sinkClear(); const r1 = await s.api("POST", "/api/collections/users/request-otp", { email: USER.email }); const [m] = await sinkWait(1); const r = await s.api("POST", `/api/collections/users/auth-with-otp?mfaId=${s.st.mfaId}`, { otpId: r1.json?.otpId, password: otpFrom(m) }); s.st.userToken = String(r.json?.token ?? ""); return { status: r.status, ...authShape(r.json) }; });
