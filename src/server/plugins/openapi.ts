...
function superuserPaths(paths: Record<string, Record<string, Operation>>) {
  ...
  add("/api/backups", "get", op(tag, "List backups", "Superusers only.", { responses: { "200": { description: "The backups", ...json({ type: "array", items: { type: "object", properties: { key: str(), size: { type: "integer" }, modified: str() } } }) }, "403": errors["403"] } }));
  add("/api/backups", "post", op(tag, "Take a backup", "Superusers only. One at a time: /api/health says canBackup.", { requestBody: { ...json({ type: "object", properties: { name: str({ description: "optional file name ending in .zip" }) } }) }, responses: { "204": { description: "Started" }, "400": errors["400"], "403": errors["403"] } }));
  ...
  add("/api/backups/{key}/restore", "post", op(tag, "Restore a backup", "Superusers only. Replaces the instance's data with the backup's.", { parameters: [path("key", "the backup file name")], responses: { "204": { description: "Started" }, "400": errors["400"], "403": errors["403"], "404": errors["404"] } }));
