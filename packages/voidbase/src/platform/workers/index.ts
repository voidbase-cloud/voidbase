// `@voidbase-cloud/voidbase/platform` on workerd: the runtime env and the logger, the two platform picks nearly
// every plugin makes. Inside this package these are `#platform/env` and `#platform/log`, which resolve only here;
// a plugin in its own package imports this name and gets the same two files under the same `workerd` condition.
//
