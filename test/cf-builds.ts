...
// connects the repository and creates the CI Worker with its two triggers (every push builds except release-please's
// branch; variables; the release secrets on the master trigger only) and the builder Worker with its one (the
// instance builder, only through the API), idempotently; a Worker's third trigger is refused the way the live API
// refuses it; builds are triggered, listed, followed and cancelled, variables set, a project removed; release
...
try {
  ...
  const setup = cfb(["setup", "--cloud", "https://cloud.example.com/"]);
  ...
  check("setup: repository connected, the CI Worker and the builder Worker with tags, three triggers", setup.code === 0 && s.connections.length === 1 && s.connections[0].repo_id === "1359087906" && s.connections[0].provider_account_name === "voidbase-cloud" && !!tagOf("voidbase-ci") && !!tagOf("voidbase-builder") && (await scripts()).length === 2 && s.triggers.length === 3, setup.out.slice(0, 400));
  const prod = byName("voidbase-ci (master)"), preview = byName("voidbase-ci (branches)"), builder = byName("voidbase-builder (instance-build)");
  ...
  check("the CI preview trigger: every other branch but release-please's (by its exact name: no wildcards), `bun run version` for a preview URL", preview?.external_script_id === tagOf("voidbase-ci") && preview.deploy_command === "bun run version" && JSON.stringify(preview.branch_includes) === '["*"]' && JSON.stringify(preview.branch_excludes) === '["master","release-please--branches--master--components--voidbase"]', JSON.stringify(preview));
  check("every push builds: the CI triggers' watch paths exclude nothing", [prod, preview].every((t) => JSON.stringify(t.path_includes) === '["*"]' && JSON.stringify(t.path_excludes) === '[]'), JSON.stringify(s.triggers.map((t: { path_excludes: string[] }) => t.path_excludes)));
  check("the instance builder: its own Worker (a third trigger on one Worker is refused), master's code, scripts/instance-build.ts, never on push", builder?.external_script_id === tagOf("voidbase-builder") && builder.build_command === "bun scripts/instance-build.ts" && JSON.stringify(builder.branch_includes) === '["master"]' && JSON.stringify(builder.path_excludes) === '["*"]' && builder.build_caching_enabled === false, JSON.stringify(builder));
  check("the builder's variables: the control plane, its superuser as secrets, BUN_VERSION; no release secrets", s.buildEnv[builder.trigger_uuid]?.VB_CLOUD_URL?.value === "https://cloud.example.com" && s.buildEnv[builder.trigger_uuid]?.VB_BUILD_EMAIL?.is_secret === true && s.buildEnv[builder.trigger_uuid]?.VB_BUILD_PASSWORD?.value === "build-pass" && s.buildEnv[builder.trigger_uuid]?.BUN_VERSION?.value === "1.3.14" && !s.buildEnv[builder.trigger_uuid]?.GH_TOKEN && !s.buildEnv[builder.trigger_uuid]?.NPM_TOKEN, JSON.stringify(s.buildEnv[builder?.trigger_uuid]));
  ...
  check("the status record URL points at the canonical domain", s.buildEnv[prod.trigger_uuid]?.CI_STATUS_URL?.value === "https://release.voidbase.cloud/status.json" && s.buildEnv[preview.trigger_uuid]?.CI_STATUS_URL?.value === "https://release.voidbase.cloud/status.json", JSON.stringify(s.buildEnv[prod.trigger_uuid]));
  ...
  check("setup's summary names what it created, the secrets, the builder and how to start the first build", /created/.test(setup.out) && /release secrets on the master trigger: GH_TOKEN, NPM_TOKEN/.test(setup.out) && /instance builds on voidbase-builder: created; build `bun scripts\/instance-build\.ts` for https:\/\/cloud\.example\.com, started by the control plane, never by a push; builder superuser stored/.test(setup.out) && /Every push to voidbase-cloud\/voidbase builds/.test(setup.out) && /cf-builds\.ts build --branch master --follow/.test(setup.out) && !/\.github/.test(setup.out), setup.out.slice(-700));
  const again = cfb(["setup"]);
  s = await state();
  check("setup again: nothing duplicated, triggers updated in place, the builder's control plane defaults to voidbase.cloud", again.code === 0 && s.connections.length === 1 && s.triggers.length === 3 && (await scripts()).length === 2 && /updated/.test(again.out) && /exists/.test(again.out) && s.buildEnv[builder.trigger_uuid]?.VB_CLOUD_URL?.value === "https://voidbase.cloud", again.out.slice(0, 300));
  ...
  check("a Worker's third trigger is refused with 12030, the way the live API refuses it", third.status === 400 && /12030/.test(await third.text()), String(third.status));
  const status = cfb(["status"]);
  check("status lists both projects, their triggers and no builds yet", status.code === 0 && /voidbase-ci \(tag/.test(status.out) && /voidbase-builder \(tag/.test(status.out) && (status.out.match(/trigger [0-9a-f-]{36}/g) ?? []).length === 3 && /no builds yet/.test(status.out), status.out);
  ...
  check("remove: the old release project's trigger and Worker are gone", removed.code === 0 && /trigger .* removed/.test(removed.out) && /Worker voidbase-release removed/.test(removed.out) && s.triggers.length === 3 && (await scripts()).length === 2, removed.out);
