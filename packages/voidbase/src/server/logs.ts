...
async function logRequest(c: Context<AppEnv>, started: number, err: unknown): Promise<void> {
  ...
  // Analytics Engine is not written here. The observability plugin owns the sampling (plugins/observability.ts,
  // providing observability@1): it wraps the request path through a slot of app.ts's and writes one data point per
  // request keyed by the matched route, so a replacement plugin can send the same measurements somewhere else.
