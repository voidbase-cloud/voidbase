...
mkdirSync(`${dir}/pb_secrets`); writeFileSync(`${dir}/pb_secrets/main.ts`, `import { defineSecrets, describe, local, string } from ${JSON.stringify(resolve(import.meta.dir, "../src/env/define.ts"))};\nexport default defineSecrets({ SMTP_PASSWORD: describe(string().secret(), "the SMTP password"), WEBHOOK_TOKEN: string().secret(), ADMIN_EMAILS: string().default("root@example.com"), DEPLOY_NOTE: local(string().default("never deployed")) });\n`); writeFileSync(`${dir}/pb_secrets/secrets.json`, JSON.stringify({ SMTP_PASSWORD: "smtp-secret" }));
