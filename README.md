...
| `packages/plugin-*` | one shipped plugin each, extracted into a package of its own: [`realtime`](packages/plugin-realtime) (7.5, the template) and the nine leaves of 7.6 — [`domains`](packages/plugin-domains), [`backups`](packages/plugin-backups), [`hardening`](packages/plugin-hardening), [`shipping-flat`](packages/plugin-shipping-flat), [`tax-flat`](packages/plugin-tax-flat), [`previews`](packages/plugin-previews), [`translations`](packages/plugin-translations), [`mail`](packages/plugin-mail) and [`observability`](packages/plugin-observability). The core depends on each and loads it, so an instance is unchanged; `@voidbase-cloud/voidbase/plugins/<name>` stays published as a re-export of the package. |
