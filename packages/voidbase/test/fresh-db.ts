...
const preview = Bun.spawn(["setsid", VP, "preview", "--port", String(port), "--host", "127.0.0.1", "--strictPort"], { env: testEnv, stdout: Bun.file(logPath), stderr: Bun.file(logPath) });
let pass = 0, fail = 0;
...
try {
  ...
  await $`${VP} build`.quiet(); // leave dist/ built from the project's own configuration
}
console.log(`\n${pass} pass, ${fail} fail`);
