...
const MOCK = "http://127.0.0.1:5197"; const BIN = resolve(import.meta.dir, "../bin/voidbase.ts");
...
const run = (args: string[], env: Record<string, string> = {}) => { const p = Bun.spawnSync(["bun", BIN, ...args], { cwd: dir, env: { ...process.env, CLOUDFLARE_API_BASE: MOCK, VOIDBASE_DEPLOY_CF_API_KEY: "", CLOUDFLARE_API_TOKEN: "", VOIDBASE_SUPERUSER_EMAIL: "", VOIDBASE_SUPERUSER_PASSWORD: "", PB_SUPERUSER_EMAIL: "", PB_SUPERUSER_PASSWORD: "", ...env } }); return { code: p.exitCode, out: new TextDecoder().decode(p.stdout) + new TextDecoder().decode(p.stderr) }; };
try {
  ...
  const cfg = existsSync(`${dir}/cloud/wrangler.jsonc`) ? readFileSync(`${dir}/cloud/wrangler.jsonc`, "utf8") : "";
  ...
  check("first dry run: resources created, cloud/ + wrangler.jsonc written", first.code === 0 && /D1 .*-db created/.test(first.out) && /R2 .*-storage created/.test(first.out) && !!parsed && parsed.account_id === "acc123" && /^[0-9a-f-]{36}$/.test(parsed.d1_databases[0]!.database_id) && parsed.r2_buckets[0]!.bucket_name === `${parsed.name}-storage` && existsSync(`${dir}/cloud/vite.config.ts`), first.out.slice(0, 300));
  check("worker name derives from the parent directory (vb -> <parent>-backend)", !!parsed && parsed.name === "shopdemo-backend", parsed?.name ?? "");
  check("dry run stops before installing and deploying, reports the workers.dev url", first.out.includes("dry run") && first.out.includes(".testsub.workers.dev") && !existsSync(`${dir}/cloud/node_modules`), first.out.slice(-200));
  const creds = JSON.parse(readFileSync(`${dir}/cloud/.superuser-credentials`, "utf8")) as { email: string; password: string };
  check("superuser credentials generated once and git-ignored", creds.email === "admin@example.com" && creds.password.length === 20 && readFileSync(`${dir}/cloud/.gitignore`, "utf8").includes(".superuser-credentials"), JSON.stringify(creds));
  const second = run(["deploy", "--dry-run"], { VOIDBASE_DEPLOY_CF_API_KEY: "cf-test-token" });
  const creds2 = JSON.parse(readFileSync(`${dir}/cloud/.superuser-credentials`, "utf8")) as { password: string };
  check("second run is idempotent: resources exist, same ids, same password", second.code === 0 && /D1 .* exists/.test(second.out) && /R2 .* exists/.test(second.out) && readFileSync(`${dir}/cloud/wrangler.jsonc`, "utf8") === cfg && creds2.password === creds.password, second.out.slice(0, 200));
  ...
  const named2 = JSON.parse(readFileSync(`${dir}/cloud/wrangler.jsonc`, "utf8").replace(/^\/\/.*\n/, "")) as { name: string };
  const creds3 = JSON.parse(readFileSync(`${dir}/cloud/.superuser-credentials`, "utf8")) as { email: string; password: string };
