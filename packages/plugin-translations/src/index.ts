...
//
// The file is the one that sat in `packages/voidbase/src/server/plugins/translations.ts`, with its ten relative
// imports written as the six published names they already resolved to. `#platform/env` and `#platform/log` are
// package-private and are one entry outside the core, `@voidbase-cloud/voidbase/platform`, which keeps both runtime
// conditions; `../auth-slot`, `../collections/model`, `../db` and `../errors` are four modules behind `/sdk`;
// `../kernel`, `../types` and `./manifest` keep their own names. The tenth is the interesting one: `./collections`
// was a sibling file and is `@voidbase-cloud/voidbase/plugins/collections` now -- the same entry a marketplace
// plugin that owns a collection imports, so this package declares its own the way any other plugin does.
import type { Context, Hono } from "hono";
import { env as runtimeEnv, logger } from "@voidbase-cloud/voidbase/platform";
import { all, badRequest, ident, listCollections, one, requireSuperuser } from "@voidbase-cloud/voidbase/sdk";
import { onAfterRead, onBootstrap, type Kernel, type RecordsRead } from "@voidbase-cloud/voidbase/kernel";
