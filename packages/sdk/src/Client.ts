...
/**
 * Whether a value an onError hook returned is a reply to parse as a Response
 * (anything with a `json()` method and a numeric `status`).
 */
function isResponseLike(v: any): v is Response {
    return (
        typeof v === "object" &&
        v !== null &&
...
export default class Client {
    ...
    /**
     * Hook lists on the request path, for plugins and for more than one
     * hook of a kind (the `beforeSend` and `afterSend` properties above take
     * one function each and keep working as documented; they run first).
     *
     * - `hooks.beforeSend.add(fn)`: called with the url and request options
     *   right before the fetch request; may modify the options in place or
     *   return `{ url, options }` to replace them.
     * - `hooks.afterSend.add(fn)`: called with the response and its parsed
     *   data; what it returns becomes the data.
     * - `hooks.onError.add(fn)`: called with the `ClientResponseError` about
     *   to be thrown (network failures have status 0); may return a `Response`
     *   or a data value to recover the request instead.
     *
     * Every `add` returns the remover of what it added.
     *
     * Example:
     * ```js
     * const remove = pb.hooks.beforeSend.add((url, options) => {
     *     options.headers = Object.assign({}, options.headers, { "X-Trace": "1" });
     * });
     *
     * remove(); // when done
     * ```
    ...
    readonly hooks: ClientHooks = {
        beforeSend: new HookList<BeforeSendHook>(),
        afterSend: new HookList<AfterSendHook>(),
        onError: new HookList<OnErrorHook>(),
    };
    ...
    private installedPlugins: Array<{ name: string; uninstall?: () => void }> = [];
    ...
    /**
     * Installs a plugin: calls its `install(client)` once and remembers the
     * returned uninstall function, if any, for `unuse(name)`.
     *
     * Throws if a plugin of the same name is already installed.
     *
     * Returns the client, typed with what the plugin attaches to it.
     *
     * Example:
     * ```js
     ...
     * ```
     */
    use<E extends object = {}>(plugin: Plugin<E>): this & E {
        if (this.installedPlugins.some((p) => p.name === plugin.name)) {
        ...
        this.installedPlugins.push({
            name: plugin.name,
            uninstall: typeof uninstall === "function" ? uninstall : undefined,
        });

        return this as this & E;
    }

    /**
     * Uninstalls the plugin with that name (calls the function its `install`
     * returned, if any). Does nothing if no such plugin is installed.
     */
    unuse(name: string): this {
        const i = this.installedPlugins.findIndex((p) => p.name === name);
        if (i >= 0) {
            const [installed] = this.installedPlugins.splice(i, 1);
            installed.uninstall?.();
        }
    ...
    /**
     * The names of the installed plugins, in installation order.
     */
    get plugins(): Array<string> {
        return this.installedPlugins.map((p) => p.name);
    }
    ...
    async send<T = any>(path: string, options: SendOptions): Promise<T> {
        ...
        // the beforeSend hook list runs after the beforeSend property
        for (const hook of this.hooks.beforeSend) {
            const result = await hook(url, options);
            if (
                result &&
                (typeof result.url !== "undefined" ||
                    typeof result.options !== "undefined")
        ...
        try {
            ...
            const wrapped = new ClientResponseError(err);

            // the onError hook list may recover the request
            for (const hook of this.hooks.onError) {
                const recovered = await hook(wrapped, { url, options });
                if (typeof recovered === "undefined") {
                    continue;
                }

                if (isResponseLike(recovered)) {
                    return this.parseResponse<T>(recovered, options).catch((e) => {
    ...
    /**
     * Parses the response of a request into the data `send()` resolves with,
     * running the afterSend property and hook list and throwing on 4xx/5xx.
     */
    private async parseResponse<T>(response: Response, options: SendOptions): Promise<T> {
        let data: any = {};
        ...
        // the afterSend hook list runs after the afterSend property
        for (const hook of this.hooks.afterSend) {
            data = await hook(response, data, options);
        }
