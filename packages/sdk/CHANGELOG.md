...
Added, on the same upstream base (PocketBase JS SDK 0.28.1):

- Reads answered from a local store, in the `offline` plugin: `offline({ cache: { collections, store, mode, max } })` writes every successful `getOne` and `getList` through to the store, answers from it when the network refuses (or straight away in `stale-while-revalidate` mode, revalidating behind it and emitting a `cache` event when the answer moved), keeps it fresh from realtime events and from the queue's own optimistic answers, and caps each collection at `max` records. A cached answer is marked, so the caller can tell. It is a cache for rendering, not a local database.

- In-place editing asks before it offers: `editable` calls the instance's `can-update` route for a record before making an element editable, and leaves it alone when the caller may not write that field; an instance without the route behaves as before, enabling the element and reverting a refused save. Markdown mode gained a rendered preview beside the source, from the `render` the caller passes (`preview: true | false | "toggle"`).

- A chat widget the page can mount: `mountChat(target, { client, conversation, system, model, stream, realtime })` from `@voidbase-cloud/sdk/ai` builds a small dependency-free thread over `client.ai.conversations`, streaming the answer, showing the tool steps it used, and putting the text back in the box when a turn fails.
...
* released with @voidbase-cloud/voidbase 0.9.0-beta.69
...
- Conversations on the server, on `client.ai.conversations` (same `@voidbase-cloud/sdk/ai` entry point): `create`, `list`, `get` and `remove` over the instance's `/api/ai/conversations` routes for a signed-in user, and `open(id)` or `start(opts)` answering a `ServerConversation` whose `send(content)` appends a turn and its reply, `stream(content, { onDelta })` reads the streamed final answer, `subscribe(cb)` follows the conversation over realtime on `ai_messages` so a reply landing from another tab reaches `messages`, and `refresh()` and `remove()` do what they say.

## 0.4.0

Added, on the same upstream base (PocketBase JS SDK 0.28.1):

- A payments helper as a plugin, `payments(options)` from the entry point `@voidbase-cloud/sdk/payments`: `client.payments.provider()` reads which provider the instance runs (`stripe`, `polar`, `lemonsqueezy` or `none`, from `/api/plugins`, or the `provider` option for callers who are not superusers), `checkout(items, { success, cancel, mode })`, `portal({ return })` and `cancel(subscription, { now })` call that provider's routes under `/api/payments/<provider>/`, `redirectToCheckout` and `redirectToPortal` send the browser there, and `mine()` lists the signed-in user's customer, subscriptions and payments.

- An SEO helper as a plugin, `seo()` from the entry point `@voidbase-cloud/sdk/seo`: `client.seo.meta({ path } | { collection, id })` fetches a record's page metadata from `/api/seo/meta` (canonical, title, description, image, JSON-LD, OpenGraph and Twitter tags, hreflang alternates), `apply(meta)` writes the tags into the document head idempotently, `shareImage(collection, id, format)` is the URL of the rendered share card, and the standalone `seoHead(meta)` returns the head fragment for server rendering.

## 0.3.0

Added, on the same upstream base (PocketBase JS SDK 0.28.1):

- Markdown editing inside the `editable` plugin: an element marked `data-vb-edit-markdown` beside its `data-vb-edit` edits the field's markdown source in place, in a textarea with a small toolbar (bold, italic, heading, link, list, code, Save and Cancel; Ctrl/Cmd+B, I and K; Escape cancels), fetched once per element, saved through the same path and states as the text mode, and rendered back through the `render(markdown)` option when the application passes one (no renderer is bundled).

- A chat helper as a plugin, `ai(options)` from the entry point `@voidbase-cloud/sdk/ai`: `client.ai.chat(messages, { model, tools, maxSteps, signal })` calls the instance's `POST /api/ai/chat` with the client's token and hooks, and `client.ai.conversation({ key, system })` keeps a history in memory, or in `sessionStorage` with `persist: true`; a refused turn leaves the history untouched.

## 0.2.0

Added, on the same upstream base (PocketBase JS SDK 0.28.1):

- PWA helpers from the entry point `@voidbase-cloud/sdk/pwa`: `registerServiceWorker(options)` registers the adapter's `sw.js` after `load`, offers an update through `onUpdate(apply)` (or applies it with `immediate`) where `apply` posts `SKIP_WAITING` and reloads once on `controllerchange`, reports `UPDATED` messages through `onUpdated`, and answers `{ registration, update, unregister, version }` or `null` where unsupported; `installPrompt()` captures `beforeinstallprompt`; `pwa(options)` is the same as a plugin, with the pending registration as `client.pwa`.

- In-place editing as a plugin, `editable(options)` from the entry point `@voidbase-cloud/sdk/editable`: elements marked `data-vb-edit="collection:id:field"` are `contenteditable` while the client is authenticated (elements added later are observed), and what is typed is saved through `collection(c).update(id, { [field]: text })` after a pause, on blur and on Enter (`data-vb-edit-multiline` keeps Enter), with `data-vb-state="saving|saved|error"` on the element; Escape and a refused save revert; `canEdit`, `onSaved`, `onError`, `debounce`, `idleTimeout`, `attribute`, `root` and an opt-in `realtime` subscription per collection to the touched ids; `client.editable` with `enabled`, `refresh()`, `flush()`.

## 0.1.0

First release of `@voidbase-cloud/sdk`, a fork of the PocketBase JavaScript SDK. It tracks PocketBase JS SDK 0.28.1: the default export and every named export are upstream's, unchanged, and the entries below this one are upstream's changelog.

Added on top of upstream:

- `VoidBase<Collections>`, a `Client` whose `collection(name)` is typed per collection from the `Collections` interface that `voidbase types` writes; a name the map does not know answers the untyped `RecordService` as before. Exported types `CollectionMap`, `AnyCollections`, `CollectionMapOf`.

- A plugin surface: `client.use(plugin)`, `client.unuse(name)`, `client.plugins`, with `Plugin { name; install(client): void | uninstall }` (`Plugin<Ext>` types what a plugin attaches to the client).

- Hook lists on the request path: `client.hooks.beforeSend.add(fn)`, `client.hooks.afterSend.add(fn)`, `client.hooks.onError.add(fn)`, each `add` returning the remover (`HookList` with `remove`, `clear`, `size`, `list`). `onError` hooks may recover a failed request by returning a `Response` or data. The upstream `beforeSend`/`afterSend` properties keep working and run first.

- An offline queue as a plugin, `offline()` from the second entry point `@voidbase-cloud/sdk/offline`: record mutations the network refuses (or attempted while `navigator.onLine` is false) are queued in `localStorage`, an injected `{ get, set }` store, or memory, answered optimistically (a create with a 15-character id chosen up front), and replayed in order on the `online` event and on `client.offline.flush()`; `pending()`, and the events `queued`, `flushed`, `failed` (a 4xx on replay drops the mutation). Reads through a local store are out of scope.

- The build runs on Bun (`bun run build`, rollup under `bun --bun`), which sidesteps the Node <= 21 requirement of `rollup-plugin-ts`; `bun.lock` is committed.
