...
//
// The file is the one that sat in `packages/voidbase/src/server/plugins/commerce.ts`, with its relative imports
// written as the published names they already resolved to: `#platform/env` and `#platform/log` are `/platform`, the
// core's helpers are `/sdk` (`RecordContext` is `/types`), and `./collections`, `./manifest` and `./payments-shared`
// are `/plugins/collections`, `/plugins` and `/plugins/payments-shared`. Nothing else about the plugin changed.
import { env as voidEnv, logger } from "@voidbase-cloud/voidbase/platform";
import { isSuperuser, requireAuth, requireSuperuser } from "@voidbase-cloud/voidbase/sdk";
import { findCollection, loadCollections } from "@voidbase-cloud/voidbase/sdk";
import { collectionId } from "@voidbase-cloud/voidbase/sdk";
import { all, ident } from "@voidbase-cloud/voidbase/sdk";
import { ApiError, badRequest, forbidden, notFound } from "@voidbase-cloud/voidbase/sdk";
import { onBootstrap, serve, using } from "@voidbase-cloud/voidbase/kernel";
import { createRecord, deleteRecord, PreconditionFailed, updateRecord } from "@voidbase-cloud/voidbase/sdk";
import { rowToValues } from "@voidbase-cloud/voidbase/sdk";
import { bufferedTransaction } from "@voidbase-cloud/voidbase/sdk";
import { ensureCollections } from "@voidbase-cloud/voidbase/plugins/collections";
import { chargedBeforeProviderTax, obj, onPaymentWritten, paymentReference, purchasedItems, str } from "@voidbase-cloud/voidbase/plugins/payments-shared";
...
// ---- the rows ------------------------------------------------------------------------------------------------
export const COMMERCE_COLLECTIONS = ["products", "variants", "inventory", "carts", "cart_items", "orders", "order_items", "shipments", "refunds", "commerce_audit"];
...
export function d1Rows(env, realtime) {
  let pending;
  ...
  const collection = async (name) => {
    ...
    if (!c)
      throw new ApiError(503, `the commerce plugin's collection "${name}" does not exist yet; it is created on the first request once ${COMMERCE_VAR} is set`);
    return c;
  };
  const where = (w) => {
  ...
  return {
    ...
    async list(name, w) {
      ...
      const q = where(w);
      const rows = await all(env.DB, `SELECT * FROM ${ident(name)}${q.sql ? ` WHERE ${q.sql}` : ""} ORDER BY rowid ASC`, q.params);
      return rows.map((r) => rowToValues(c, r));
    },
    async create(name, values) { return (await createRecord(await ctx(), await collection(name), values, {})); },
    async update(name, id, values) { return (await updateRecord(await ctx(), await collection(name), id, values, {})); },
    async updateWhere(name, id, w, values) {
      ...
      try {
        await updateRecord(await ctx(), await collection(name), id, values, { where: w });
        return true;
      }
      catch (e) {
    ...
    async createOnce(name, values, alongside = []) {
      ...
      const base = await ctx(), held = [];
      try {
        if (!alongside.length) {
          await createRecord(base, await collection(name), values, {});
          return true;
        }
        const tx = bufferedTransaction(env.DB);
        const inside = { ...base, db: tx.db, realtime: holding(base.realtime, held), changes: undefined };
        for (const u of alongside)
          await updateRecord(inside, await collection(u.collection), u.id, u.values, {});
      ...
      if (held.length)
        await base.realtime.publish(held);
      return true;
    },
    ...
    async customers(user) {
      ...
      if (!c)
        return [];
      const rows = await all(env.DB, "SELECT * FROM `customers` WHERE `user` = ?", [user]);
      return rows.map((r) => rowToValues(c, r));
    },
  };
}
 ...
 */
export async function collectionDefinitions(db) {
...
const lineTotal = (l) => l.quantity * l.unitPrice;
export const subtotalOf = (lines) => lines.reduce((sum, l) => sum + lineTotal(l), 0);
const quoteItems = (lines) => lines.map((l) => ({ variant: str(l.variant.id), sku: str(l.variant.sku), title: str(l.variant.title), quantity: l.quantity, unitPrice: l.unitPrice, ...(n(l.variant.weight) ? { weight: n(l.variant.weight) } : {}) }));
/** the address as a destination: the fields the two interfaces know, anything else dropped */
const ADDRESS_FIELDS = ["line1", "line2", "city", "region", "postcode", "country"];
export function addressOf(v) {
  const raw = obj(v);
  const out = {};
  for (const f of ADDRESS_FIELDS)
    if (str(raw[f]))
      out[f] = f === "country" ? str(raw[f]).toUpperCase() : str(raw[f]);
  return out;
}
const hasAddress = (a) => Object.keys(a).length > 0;
/** what is left of a variant's stock: a variant with no inventory row is not tracked, and never blocks a sale */
export async function available(rows, variant) {
  const inv = await rows.find("inventory", { variant });
  return inv ? n(inv.onHand) - n(inv.reserved) : null;
}
async function reserve(rows, variant, quantity) {
  const inv = await rows.find("inventory", { variant });
  if (!inv)
    return;
  await rows.update("inventory", str(inv.id), { reserved: n(inv.reserved) + quantity });
}
async function release(rows, variant, quantity) {
  const inv = await rows.find("inventory", { variant });
  if (!inv)
    return;
...
async function ship(rows, variant, quantity) {
  const inv = await rows.find("inventory", { variant });
  if (!inv)
    return;
...
const randomNumber = () => `VB-${Date.now().toString(36).toUpperCase()}-${Math.floor(Math.random() * 36 ** 3).toString(36).toUpperCase().padStart(3, "0")}`;
export const defaultDeps = () => ({ rows: (env, realtime) => d1Rows(env, realtime), now: () => Date.now(), token: randomToken, number: randomNumber });
export function commerceWith(overrides = {}) {
  const deps = { ...defaultDeps(), ...Object.fromEntries(Object.entries(overrides).filter(([, v]) => v !== undefined)) };
  const iso = () => new Date(deps.now()).toISOString();
  const mustBeOn = (env) => {
    if (!commerceOn(env))
      throw new ApiError(503, `The shop is not turned on on this instance: set ${COMMERCE_VAR}=1`);
  };
  // --- the audit trail ---------------------------------------------------------------------------------------
  const audit = async (rows, actor, action, subject, detail) => {
    await rows.create("commerce_audit", { at: iso(), actor, action, subject, detail });
  };
  const actorOf = (auth) => (auth ? `${auth.collection.name}:${str(auth.row.id)}` : "anonymous");
  // --- the cart ----------------------------------------------------------------------------------------------
  const tokenOf = (c, body = {}) => str(c.req.header(CART_TOKEN_HEADER) || c.req.query("token") || body.token);
  /** the caller's open cart, found by their session or by the token they carry; created only when asked */
  async function cartOf(rows, c, o) {
    ...
    if (user) {
      ...
      const theirs = await rows.find("carts", { token, status: "open" });
      if (theirs)
        return str(theirs.user) ? null : theirs;
    }
    if (!o.create)
      return null;
  ...
  async function linesOf(rows, collection, key, id) {
  ...
  let ctx;
  const need = (iface) => {
    const found = ctx ? using(ctx, iface) : undefined;
    if (!found)
      throw new ApiError(503, `the commerce plugin requires "${iface}" and nothing provides it on this instance`);
    return found;
  };
   ...
   */
  const pays = (payment, order) => chargedBeforeProviderTax(payment) === n(order.total) && str(payment.currency).toLowerCase() === str(order.currency).toLowerCase();
  const paidOf = (payment) => {
    const before = chargedBeforeProviderTax(payment), amount = n(payment.amount), currency = str(payment.currency);
    if (!Number.isFinite(before))
      return `${amount} ${currency} with the provider's own tax (what it charged before that tax cannot be read)`;
    return before === amount ? `${amount} ${currency}` : `${before} ${currency} before the provider's own tax (${amount} ${currency} with it)`;
  };
  const owes = (order) => ({ order: str(order.id), total: n(order.total), currency: str(order.currency) });
  /**
   ...
   */
  const subscriptionMoney = (payment) => {
  ...
  const subjectOf = (order) => `order:${str(order.id)}`;
  /**
  ...
  async function auditOnce(rows, id, actor, action, subject, detail, alongside) {
  ...
  async function noteOnce(rows, actor, action, subject, detail) {
   ...
   */
  async function moveRow(rows, actor, order, action, detail) {
   ...
   */
  const orderedOf = (lines) => lines.map((l) => {
  ...
  const itemsSaid = (items) => items.map((i) => (i.quantity === undefined ? `"${i.id}"` : `${i.quantity} of "${i.id}"`)).join(" and ") || "nothing that could be read";
  /**
   ...
   */
  const boughtTheOrder = (bought, ordered) => {
    if (bought.length !== ordered.length)
      return false;
    const left = [...ordered];
   ...
   */
  async function claim(rows, order, from, to, payment) {
    const id = str(order.id);
    if (await rows.updateWhere("orders", id, { status: from }, { status: to, payment }))
      return;
    const now = await rows.find("orders", { id });
    if (str(now?.status) === to && str(now?.payment) === payment)
      return;
   ...
   */
  async function stockStep(rows, actor, order, action, payment, line) {
  ...
  async function reserveBack(rows, actor, order, payment) {
    ...
    for (const l of await linesOf(rows, "order_items", "order", id)) {
      if (released.has(str(l.item.id)) && !reserved.has(str(l.item.id)))
        await stockStep(rows, actor, order, "stock.reserved", payment, l);
    }
  }
  /** the `order.oversold` note, once per order and payment, naming each line a success reserved above what was on hand */
  async function oversoldOnce(rows, actor, order, payment) {
   ...
   */
  async function failedMove(rows, actor, order, detail) {
    const id = str(order.id), payment = str(detail.payment);
    const now = await rows.find("orders", { id });
    if (str(now?.status) === "cancelled" && str(now?.payment) === payment) {
      for (const l of await linesOf(rows, "order_items", "order", id))
        await stockStep(rows, actor, order, "stock.released", payment, l);
   ...
   */
  async function finishRevive(rows, actor, order) {
    const after = await rows.find("orders", { id: str(order.id) });
    if (!after || str(after.status) !== "paid" || !str(after.payment))
      return;
   ...
   */
  async function retoldFailure(rows, actor, order, detail) {
    await moveRow(rows, actor, order, "order.payment_failed", detail);
    await finishRevive(rows, actor, order);
  }
  /**
   ...
   */
  async function paidMove(rows, actor, order, detail, cancelledBy) {
    ...
    if (cancelledBy === undefined) {
      ...
      const failure = (action) => str(obj(trail.find((r) => str(r.action) === action)?.detail).payment);
    ...
    if (by)
      await oversoldOnce(rows, actor, order, payment);
  }
  /**
   ...
   */
  async function revive(rows, actor, order, detail) {
   ...
   */
  async function cancelledBefore(rows, order, payment) {
   ...
   */
  async function orderForPayment(rows, payment, reference, named) {
    ...
    const judged = await (async () => {
      if (!named)
        return { why: `it names order "${reference}", which does not exist`, note: {} };
      if (!customer || str(named.customer) !== customer)
        return { why: `it names order "${reference}", which is another customer's`, note: {} };
      if (revive)
        return whyNotRevived(rows, named, payment, reference);
      if (status !== "pending")
        return { why: `it names order "${reference}", which is ${status} and not pending`, note: {} };
      if (failed) {
      ...
      if (!pays(payment, named))
        return { why: `it names order "${reference}" and pays ${paidOf(payment)}, where the order's total is ${n(named.total)} ${str(named.currency)}`, note: {} };
   ...
   */
  async function whyNotRevived(rows, order, payment, reference) {
    const id = str(order.id);
    const was = str(order.payment) === str(payment.id) ? `it cancelled order "${id}" when it failed` : `it names order "${id}", which is cancelled`;
    if (reference !== id)
      return { why: `${was}, and now ${reference ? `names order "${reference}"` : "names no order"}`, note: {} };
    if (!str(order.payment))
      return { why: `${was} and carries no payment, so no failed payment cancelled it: a refused checkout (order.checkout_failed) cancels an order before it has one`, note: {} };
    if (!pays(payment, order))
      return { why: `${was}, and pays ${paidOf(payment)}, where the order's total is ${n(order.total)} ${str(order.currency)}`, note: {} };
  ...
  async function movedBy(rows, actor, payment, order, detail, reference) {
    ...
    if (status === "failed") {
      if (await cancelledBefore(rows, order, str(payment.id)))
        return retoldFailure(rows, actor, order, detail);
  ...
  async function onPayment(e) {
    ...
    if (subscriptionMoney(e.payment))
      return;
    ...
    if (moved)
      return movedBy(rows, actor, e.payment, moved, detail, reference);
    const named = await rows.find("orders", { id: reference });
    if (named && str(named.payment) === payment)
      return movedBy(rows, actor, e.payment, named, detail, reference);
    ...
    if (status === "failed" && named && str(named.status) !== "pending" && (await cancelledBefore(rows, named, payment)))
      return retoldFailure(rows, actor, named, detail);
    ...
    if (mends)
      return revive(rows, actor, order, detail);
    if (status === "succeeded") {
  ...
  async function quote(c, rows, cart, lines) {
    ...
    const tax = await need("tax@1").quote(c.env, { items, to: address, ...(customer ? { customer } : {}) });
    const rates = await need("shipping@1").rates(c.env, { items, to: address });
    return { address, subtotal: subtotalOf(lines), tax, rates };
  }
  /** the order a superuser route names */
  async function orderOf(rows, id) {
    const order = id ? await rows.find("orders", { id }) : null;
    if (!order)
      throw notFound(`no order "${id}"`);
    return order;
  }
  function mountRoutes(app) {
    const cartRoute = async (c) => {
      mustBeOn(c.env);
    ...
    app.patch(`${API}/cart/items/:id`, async (c) => {
      ...
      if (!Number.isInteger(quantity) || quantity < 0)
        throw badRequest("quantity must be a whole number, 0 to take the line away");
    ...
    app.post(`${API}/cart/address`, async (c) => {
      ...
      if (!hasAddress(address))
        throw badRequest("address must say where the order is going: line1, city, postcode, country");
    ...
    app.post(`${API}/checkout`, async (c) => {
      ...
      if (!success)
        throw badRequest("success must be the URL to return to");
      const rows = rowsFor(c);
      const cart = await cartOf(rows, c, { create: false, body });
      if (!cart)
        throw notFound("no open cart");
      const lines = await linesOf(rows, "cart_items", "cart", str(cart.id));
      if (!lines.length)
        throw badRequest("the cart is empty");
      if (!hasAddress(addressOf(cart.address)))
        throw badRequest(`POST ${API}/cart/address first: tax and shipping are quoted against a destination`);
      for (const l of lines) {
        if (!str(l.variant.sku) && !str(l.variant.priceId))
          throw badRequest(`the variant "${str(l.item.variant)}" is gone; take it out of the cart`);
        await checkStock(rows, l.variant, l.quantity);
      }
      const q = await quote(c, rows, cart, lines);
      const chosen = str(body.shipping) ? q.rates.find((r) => r.id === str(body.shipping)) : q.rates[0];
      if (str(body.shipping) && !chosen)
        throw badRequest(`"${str(body.shipping)}" is not one of the shipping rates for this address`);
      ...
      try {
        ...
        for (const l of lines)
          await release(rows, str(l.variant.id), l.quantity);
    ...
    app.post(`${API}/orders/:id/fulfil`, async (c) => {
      ...
      if (str(order.status) !== "paid")
        throw new ApiError(409, `an order is fulfilled once it is paid, and this one is ${str(order.status)}`);
      ...
      for (const l of lines)
        await ship(rows, str(l.item.variant), l.quantity);
    ...
    app.post(`${API}/orders/:id/refund`, async (c) => {
      ...
      const rows = rowsFor(c);
      const order = await orderOf(rows, c.req.param("id"));
      if (str(order.status) !== "paid" && str(order.status) !== "fulfilled")
        throw new ApiError(409, `an order is refunded once it is paid, and this one is ${str(order.status)}`);
      const amount = body.amount === undefined ? n(order.total) : n(body.amount);
      if (amount <= 0 || amount > n(order.total))
        throw badRequest(`amount must be between 1 and the order's total of ${n(order.total)}`);
    ...
    app.get(`${API}/orders`, async (c) => {
      ...
      const rows = rowsFor(c);
      const mine = [];
      for (const customer of await rows.customers(str(auth.row.id)))
        mine.push(...(await rows.list("orders", { customer: str(customer.id) })));
    ...
    app.get(`${API}/orders/:id`, async (c) => {
      ...
      if (!isSuperuser(auth)) {
        ...
        if (!mine.includes(str(order.customer)))
          throw forbidden("this order belongs to somebody else");
      }
      const lines = await linesOf(rows, "order_items", "order", str(order.id));
