...
// vp (vite-plus) and void: the Cloudflare toolchain comes with the npm package, but not always in a node_modules
// beside this file. In this repository's workspace the binaries are hoisted to the root and packages/voidbase has
// no node_modules/.bin at all, so the package is resolved and the .bin next to it taken (src/node/bundle.ts and
// src/node/serve-workers.ts resolve the same way) rather than assumed at ./node_modules/.bin.
const toolchain = (pkg: string, bin: string): string => {
  let path: string | undefined;
  try { path = resolve(Bun.resolveSync(`${pkg}/package.json`, ROOT), "..", "..", ".bin", bin); } catch { /* not installed */ }
  if (!path || !existsSync(path)) { console.error(`voidbase: ${bin} not found from ${ROOT}: the Cloudflare toolchain comes with the npm package (bun install)`); process.exit(1); }
  return path;
};
...
switch (cmd) {
  ...
  case "secrets": {
    ...
    console.log(`${dir}: ${def.names.length} declared (${secretNames.length} secret, ${def.of("server").length} server, ${def.of("public").length} public${def.of("flag").length ? `, ${def.of("flag").length} flag` : ""}${def.of("local").length ? `, ${def.of("local").length} local, never deployed` : ""})${state.values ? `, ${state.provided.length} valued in secrets.json` : ", no secrets.json"}${onWorker ? `, worker "${worker}" has ${onWorker.filter((k) => secretNames.includes(k)).length} of the secrets` : " (set VOIDBASE_DEPLOY_CF_API_KEY to compare with the Worker)"}`);
  ...
  case "deploy": {
    if (flags.void) { await run(toolchain("void", "void"), ["deploy"]); break; } // the Void platform (void auth login first)
