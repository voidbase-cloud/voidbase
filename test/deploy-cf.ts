...
mkdirSync(`${dir}/pb_secrets`); writeFileSync(`${dir}/pb_secrets/main.ts`, `import { defineSecrets, local, secret, server, string } from ${JSON.stringify(resolve(import.meta.dir, "../src/env/define.ts"))};\nexport default defineSecrets({ OPTIONAL_TOKEN: secret(string().optional(), "may be absent"), SMTP_PASSWORD: secret(string(), "the SMTP password"), WEBHOOK_TOKEN: secret(string()), ADMIN_EMAILS: server(string().default("root@example.com")), DEPLOY_NOTE: local(string().default("never deployed")) });\n`); writeFileSync(`${dir}/pb_secrets/secrets.json`, JSON.stringify({ SMTP_PASSWORD: "smtp-secret" }));
...
try {
  ...
  check("an optional declared secret without a value is not a missing one", !/OPTIONAL_TOKEN/.test(dry.stdout + dry.stderr), (dry.stdout + dry.stderr).split("\n").filter((l) => /secrets:/.test(l)).join(" | ").slice(0, 300));
