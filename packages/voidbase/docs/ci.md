...
| step | what |
| --- | --- |
| install | `bun install --frozen-lockfile` (hoisted: `bunfig.toml` pins the linker, see below) |
...
`bunfig.toml` pins `[install] linker = "hoisted"`. The lockfile carries `configVersion 1`, so adding `workspaces`
to the root `package.json` is enough for Bun to switch to the isolated linker on its own, and the root's
`node_modules/.bin` then holds only the binaries of the root's own declared devDependencies. The scripts call
binaries out of that directory by path -- `scripts/pipeline.ts` runs `./node_modules/.bin/wrangler`,
`scripts/ci.sh` runs `void` and `commitlint`, `scripts/dev.sh` runs `vp` -- and `wrangler`, `drizzle-kit`,
`esbuild` and `workerd` are transitive dependencies of `void` and `vite-plus`, so under the isolated linker they
are simply absent. Measured on this repository: 12 entries in `node_modules` and no `wrangler` under the isolated
linker, 300+ and every binary under the hoisted one.
