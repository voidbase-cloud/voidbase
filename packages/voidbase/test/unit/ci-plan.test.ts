...
// the planner speaks repository-relative paths; the application and its suites are in the published package
const p = (f: string) => `packages/voidbase/${f}`;
...
describe("ci plan: hot mode", () => {
  ...
  test("the commits' scopes go first, a Tests: trailer and a changed test file are never deferred", () => {
    const d = decide(all, same, { hot: true, budget: 30, signals: { scopes: ["auth"], tests: ["hardening"], full: false, changed: [p("test/conformance/thumbs.ts")] } });
...
describe("ci plan: the import graph of this repository", () => {
  ...
  test("the deploy code reaches the CLI checks, not the Bun pass or the Workers suites", () => {
    const k = keysWith(p("src/node/deploy-cf.ts"));
  ...
  test("the server reaches every suite on both runtimes and the boots", () => {
    const k = keysWith(p("src/server/app.ts"));
  ...
  test("the Bun runtime reaches the Bun pass only", () => {
    const k = keysWith(p("src/node/serve.ts"));
  ...
  test("platform flavours: the Workers flavour reaches the Workers suites, the Bun flavour the Bun pass", () => {
    expect(keysWith(p("src/platform/workers/env.ts")).some((x) => x.startsWith("bun:"))).toBe(false);
    expect(keysWith(p("src/platform/node/env.ts")).some((x) => x.startsWith("suite:") && x !== "suite:cloud-rest")).toBe(false);
  });
  ...
  test("an extracted plugin package reaches everything that runs the instance, because the core imports it", () => {
    // The core loads `@voidbase-cloud/plugin-realtime` at module scope (src/server/app.ts), so the package is part
    ...
    expect(keysWith(p("src/server/plugins/realtime.ts"))).toContain("suite:realtime");
    // and its tsconfig, which no closure reaches, is an input to the typecheck that runs it
    expect(keysWith("packages/plugin-realtime/tsconfig.json")).toEqual(["step:typecheck"]);
  });
  test("the workspace's own package names resolve; anything else is still a dependency to stop at", () => {
    ...
    expect(t.workspaceFile("@voidbase-cloud/voidbase/platform", "bun")).toBe(p("src/platform/node/index.ts"));
    expect(t.workspaceFile("@voidbase-cloud/voidbase/nothing-like-this", "bun")).toBeNull();
    expect(t.workspaceFile("hono", "bun")).toBeNull();
    expect(t.workspaceFile("void/env", "workerd")).toBeNull();
  });
});
