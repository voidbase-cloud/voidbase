...
| `packages/plugin-*` | one shipped plugin each, extracted into a package of its own: [`realtime`](packages/plugin-realtime) (7.5, the template), [`domains`](packages/plugin-domains) , [`backups`](packages/plugin-backups) , [`hardening`](packages/plugin-hardening) and [`shipping-flat`](packages/plugin-shipping-flat). The core depends on each and loads it, so an instance is unchanged; `@voidbase-cloud/voidbase/plugins/<name>` stays published as a re-export of the package. |
