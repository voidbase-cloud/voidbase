...
// waits for n mails whose subject matches (mail leaves through the jobs queue, so a login alert from an earlier step
// can land after the sink was cleared; the subject tells the step's own mail apart)
async function sinkWait(n: number, ms = 6000, subject: RegExp = /./): Promise<Mail[]> { const until = Date.now() + ms; for (;;) { const list = ((await fetch(SINK).then((r) => r.json())) as Mail[]).filter((m) => subject.test(m.subject ?? "")); if (list.length >= n || Date.now() > until) return list; await Bun.sleep(150); } }
...
try {
  await step("request-verification sends the email", async (s) => { await sinkClear(); const r = await s.api("POST", "/api/collections/users/request-verification", { email: USER.email }); const [m] = await sinkWait(1, 15000, /verify/i); s.tokens.verification = tokenOf(m); return { status: r.status, mail: m ? normalize(m) : null }; });
  await step("immediate second request is rate limited", async (s) => { await sinkClear(); const r = await s.api("POST", "/api/collections/users/request-verification", { email: USER.email }); await Bun.sleep(1200); return { status: r.status, mails: (await sinkWait(1, 500, /verify/i)).length }; });
  await step("request-verification unknown email still 204", async (s) => { await sinkClear(); const r = await s.api("POST", "/api/collections/users/request-verification", { email: "nobody@example.com" }); await Bun.sleep(800); return { status: r.status, mails: (await sinkWait(1, 300, /verify/i)).length }; });
  ...
  await step("request-password-reset sends the email", async (s) => { await sinkClear(); const r = await s.api("POST", "/api/collections/users/request-password-reset", { email: USER.email }); const [m] = await sinkWait(1, 15000, /reset/i); s.tokens.reset = tokenOf(m); return { status: r.status, mail: m ? normalize(m) : null }; });
  ...
  await step("request-email-change sends the email", async (s) => { await sinkClear(); const r = await s.api("POST", "/api/collections/users/request-email-change", { newEmail: NEW_EMAIL }, { authorization: s.userToken }); const [m] = await sinkWait(1, 15000, /confirm/i); s.tokens.change = tokenOf(m); return { status: r.status, mail: m ? normalize(m) : null }; });
