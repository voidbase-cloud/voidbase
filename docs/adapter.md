...
The project root is Void's, with two additions, both optional and both named for the voidbase thing they are:
...
- **Static.** No `routes/`, `middleware/`, `vb_hooks/`, `crons/` or `queues/`. The generated app is `main.ts`
  plus `pb_public`.
...
resolving to a directory's index file, the three build errors (a hook attached to nothing, a misspelt hook name, a
hook left in `middleware/`), and that nothing is written outside `.voidbase/`. Run it with
