...
seedGh(); const zones = [{ id: "zone123", name: "example.com" }]; const sendingDomains = new Set<string>(); /* Email Sending: domains onboarded on zone123 */ const domains = new Map<string, { id: string; hostname: string; service: string; zone_id: string; environment: string }>(); const uploadedHashes = new Set<string>(); const pendingSessions = new Map<string, Set<string>>(); const calls: string[] = [];
...
const reset = () => { calls.length = 0; connections.clear(); triggers.clear(); builds.clear(); buildEnv.clear(); seedGh(); domains.clear(); sendingDomains.clear(); d1.clear(); d1Migrations.clear(); d1Queries.clear(); r2.clear(); queues.clear(); consumers.clear(); scripts.clear(); uploadedHashes.clear(); pendingSessions.clear(); };
