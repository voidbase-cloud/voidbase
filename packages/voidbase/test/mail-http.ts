...
// `vp build` and `vp preview`, resolved rather than assumed: this package has no node_modules/.bin of its own (the
// workspace root hoists every binary), so neither `./node_modules/.bin/vp` nor a `bun run build:app` that only the
// root manifest declares works from here. src/node/bundle.ts resolves the toolchain the same way.
const VP = resolve(Bun.resolveSync("vite-plus/package.json", import.meta.dir), "..", "..", ".bin", "vp");
...
try {
  ...
  await $`${VP} build`.env(buildEnv).quiet();
  await $`bun scripts/seed-d1.ts ${STATE}`.quiet();
  preview = Bun.spawn(["setsid", VP, "preview", "--port", String(port), "--host", "127.0.0.1", "--strictPort"], { env: { ...process.env, VOIDBASE_PERSIST_TO: STATE }, stdout: Bun.file(".void/preview-mail.log"), stderr: Bun.file(".void/preview-mail.log") });
  ...
  await $`${VP} build`.quiet(); // leave dist/ built without the test variables
}
