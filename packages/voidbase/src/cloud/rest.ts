...
export async function provisionInstance(cf: CfApi, o: ProvisionOptions): Promise<ProvisionResult> {
  ...
  // a workflow binding names a Workflow the upload does not create (wrangler deploy does, from its config): registered here,
  // or the instance's first rebuild answers workflow.not_found
  if (m.rebuild) await cf.json("PUT", `/accounts/${o.account}/workflows/${o.name}-rebuild`, { class_name: m.rebuild.className, script_name: o.name });
...
export async function destroyInstance(cf: CfApi, o: { account: string; name: string; log?: (l: string) => void }): Promise<DestroyResult> {
  ...
  await attempt(`workflow ${o.name}-rebuild`, async () => { const r = await cf.raw("GET", `/accounts/${o.account}/workflows/${o.name}-rebuild`); if (r.status === 404) return false; await cf.json("DELETE", `/accounts/${o.account}/workflows/${o.name}-rebuild`); return true; });
