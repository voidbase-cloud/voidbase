...
try {
  ...
  await step("list shows the backup", async (s) => { const r = await s.api("GET", "/api/backups", undefined, s.h); const items = ((r.json as { key: string; size: number; modified: string }[]) ?? []).filter((b) => b.key.startsWith("ks_backup_")); return { status: r.status, keys: items.map((b) => b.key), itemKeys: Object.keys(items[0] ?? {}).sort(), sizePositive: (items[0]?.size ?? 0) > 0, modifiedShape: /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d{3}Z$/.test(items[0]?.modified ?? "") }; });
