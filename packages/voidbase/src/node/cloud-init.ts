...
export function writeCloudProject(out: string, mode: "package" | "internal" = "package", extra: { hooksDir?: string; migrationsDir?: string; pluginsDir?: string; entry?: string; entryRegisters?: boolean; publicUploads?: boolean; queue?: string | false; hub?: boolean; database?: "d1" | "durable"; workflows?: { file: string; className: string }[]; seoPng?: boolean } = {}): { files: number; out: string } {
  ...
  // types are `dependencies` of this package -- a consumer installs them with it -- and were never devDependencies,
  // here or before the move to packages/voidbase, so reading devDependencies alone put `undefined` in every one of
  // them and JSON.stringify dropped the keys: the project came out with typescript and nothing else. Both maps are
  // read, as src/adapter/codegen.ts already does for the package's own spec.
  const own = JSON.parse(readFileSync(`${ROOT}/package.json`, "utf8")) as { dependencies?: Record<string, string>; devDependencies?: Record<string, string> };
  const pin = (name: string) => own.dependencies?.[name] ?? own.devDependencies?.[name];
  ...
  // pb_public uploads of a vanilla instance, from its bucket, for every path Void's router would otherwise answer. Only a
  // release is one (src/node/bundle.ts, which routes every path to the Worker first): a project stays asset-first, and
  // a global middleware would put the Worker in front of every asset it serves
  if (extra.publicUploads) files["middleware/01.pb-public.ts"] = `// A vanilla instance's pb_public uploads, served from its bucket before Void's router (src/server/public-r2.ts).\nimport { defineMiddleware } from "void";\nimport { publicUploadResponse } from "${P.cronsApp}";\n\nexport default defineMiddleware(async (c, next) => {\n  const hit = await publicUploadResponse(c.env as never, c.req.raw);\n  if (hit) return hit;\n  await next();\n});\n`;
  ...
  if (!extra.publicUploads) rmSync(resolve(out, "middleware"), { recursive: true, force: true });
