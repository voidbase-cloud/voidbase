...
export function writeCloudProject(out: string, mode: "package" | "internal" = "package", extra: { hooksDir?: string; migrationsDir?: string; pluginsDir?: string; entry?: string; queue?: string | false; hub?: boolean } = {}): { files: number; out: string } {
  ...
  const files: Record<string, string> = {
    ...
    "vite.config.ts": `import { defineConfig, loadEnv } from "vite";\nimport { voidPlugin } from "void";\nimport { pbHooksPlugin } from "${P.plugin}";\n\n// the project's pb_hooks/ and pb_migrations/ (one directory up) are bundled into the Worker\nexport default defineConfig(({ mode }) => {\n  const env = loadEnv(mode, process.cwd(), "");\n  return { plugins: [voidPlugin({ persistTo: env.VOIDBASE_PERSIST_TO || undefined }), pbHooksPlugin({ dir: env.VOIDBASE_HOOKS_DIR || ${hooksDir}, migrationsDir: env.VOIDBASE_MIGRATIONS_DIR || ${migrationsDir}, pluginsDir: env.VOIDBASE_PLUGINS_DIR || ${pluginsDir}${extra.hub !== false ? `, hubEntry: ${JSON.stringify(P.hub)}` : ""} })] };\n});\n`,
