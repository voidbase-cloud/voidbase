...
mkdirSync(`${dir}/pb_secrets`); writeFileSync(`${dir}/pb_secrets/main.ts`, `import { defineSecrets, describe, string } from ${JSON.stringify(resolve(import.meta.dir, "../src/env/define.ts"))};\nexport default defineSecrets({ SMTP_PASSWORD: describe(string().secret(), "the SMTP password"), WEBHOOK_TOKEN: string().secret(), ADMIN_EMAILS: string().default("root@example.com") });\n`); writeFileSync(`${dir}/pb_secrets/secrets.json`, JSON.stringify({ SMTP_PASSWORD: "smtp-secret" }));
...
try {
  ...
  check("voidbase secrets: a row per declared name with its tier, local value or default, and Worker presence", list.code === 0 && /3 declared \(2 secret, 1 server, 0 public\), 1 valued in secrets\.json, worker "shopdemo-backend" has 0 of the secrets/.test(list.out) && /SMTP_PASSWORD\s+secret\s+local value\s+NOT on the worker\s+the SMTP password/.test(list.out) && /WEBHOOK_TOKEN\s+secret\s+no local value/.test(list.out) && /ADMIN_EMAILS\s+server\s+default "root@example\.com"/.test(list.out), list.out.slice(0, 500));
