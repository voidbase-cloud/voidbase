...
import { alternatesOf, localizedUrl } from "@voidbase-cloud/voidbase/plugins/seo-locales";
...
export function splitOutside(spec) {
  const out = [];
  let cur = "";
  let depth = 0;
  let quote = "";
...
export function parseSeo(spec) {
  const out = [];
  for (const raw of splitOutside(spec)) {
    const m = SEO_ENTRY.exec(raw);
    if (!m) {
      logger.warn("voidbase: seo: VOIDBASE_SEO entry skipped, expected collection:Type{key=field,...}", { entry: raw });
      continue;
    }
    const fields = {};
    for (const pair of (m[3] ?? "").split(",").map((s) => s.trim()).filter(Boolean)) {
      ...
      if (!KEY.test(key) || !FIELD_PATH.test(path)) {
        logger.warn("voidbase: seo: VOIDBASE_SEO mapping skipped, expected key=field or key=relation.field", { collection: m[1], mapping: pair });
        continue;
      }
      fields[key] = path;
    }
...
export const expandOf = (mapping) => [...new Set(Object.values(mapping?.fields ?? {}).filter((p) => p.includes(".")).map((p) => p.split(".")[0]))].join(",");
const escapeRe = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
/** the first sitemap entry whose pattern matches the path, each {field} captured and URL-decoded */
export function matchPath(entries, path) {
  const p = path.length > 1 ? path.replace(/\/+$/, "") : path;
  for (const entry of entries) {
    const names = [];
    const re = new RegExp(`^${escapeRe(entry.pattern).replace(/\\\{([A-Za-z_][A-Za-z0-9_]*)\\\}/g, (_, n) => { names.push(n); return "([^/]+)"; })}$`);
    const m = re.exec(p);
    if (!m || !names.length)
      continue;
    const values = {};
    let ok = true;
    names.forEach((n, i) => { try {
      values[n] = decodeURIComponent(m[i + 1]);
    }
    catch {
      ok = false;
    } });
...
export const quoteFilter = (v) => `"${v.replace(/\\/g, "\\\\").replace(/"/g, '\\"')}"`;
/** the entry's own filter and the captured values, as one filter the records service judges with the list rule */
export const lookupFilter = (entry, values) => [...(entry.filter ? [`(${entry.filter})`] : []), ...Object.entries(values).map(([k, v]) => `${k} = ${quoteFilter(v)}`)].join(" && ");
// --- locales ------------------------------------------------------------------------------------------------------------
...
export { alternatesOf, localesOf, localizedUrl, splitLocale } from "@voidbase-cloud/voidbase/plugins/seo-locales";
export const stripTags = (s) => s.replace(/<[^>]*>/g, " ").replace(/&nbsp;/g, " ").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/\s+/g, " ").trim();
export const DESCRIPTION_MAX = 200;
/** the first 200 characters, cut back to a word boundary and closed with an ellipsis when it was longer */
export function excerpt(s, max = DESCRIPTION_MAX) {
...
function ownerOf(record, relation) {
  const expand = record.expand?.[relation];
...
const asText = (v) => v == null ? "" : Array.isArray(v) ? v.map(asText).filter(Boolean).join(", ") : typeof v === "object" ? "" : String(v);
const firstField = (collection, names, types) => names.find((n) => collection.fields.some((f) => f.name === n && types.includes(f.type) && !f.hidden)) ?? "";
...
export function pageText(collection, record, mapping) {
...
const isoOf = (v) => { const d = new Date(String(v ?? "")); return v && !Number.isNaN(d.getTime()) ? d.toISOString() : asText(v); };
...
export function buildMeta(input) {
  ...
  const ogLocale = (s) => s.replace(/-/g, "_");
  const jsonld = { "@context": "https://schema.org", "@type": type, name: title };
  ...
  for (const [key, path] of Object.entries(input.mapping?.fields ?? {})) {
    ...
    if (/^date/i.test(key))
      ...
      jsonld[key] = { "@type": key === "author" ? "Person" : "Organization", name: asText(v) };
    else
      jsonld[key] = typeof v === "object" && !Array.isArray(v) ? asText(v) : v;
  }
  const og = {
  ...
  if (locale) {
    og["og:locale"] = ogLocale(locale);
    const others = alternates.filter((a) => a.locale !== "x-default" && a.locale !== locale).map((a) => ogLocale(a.locale));
    if (others.length)
      og["og:locale:alternate"] = others;
  }
  const twitter = { "twitter:card": "summary_large_image", "twitter:title": title, "twitter:description": description, "twitter:image": image };
  const lines = [
...
// --- the share card ------------------------------------------------------------------------------------------------------
export const CARD = { width: 1200, height: 630, titleChars: 30, titleLines: 3, descriptionChars: 70 };
...
export const themeOf = (v) => (/^#(?:[0-9a-f]{3}|[0-9a-f]{6})$/i.test(v.trim()) || /^[a-z]{3,20}$/i.test(v.trim()) ? v.trim() : DEFAULT_THEME);
...
export function wrap(text, chars, lines) {
  const out = [];
  let cur = "";
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (!cur)
      cur = w;
    else if (cur.length + 1 + w.length <= chars)
      cur += ` ${w}`;
    else {
      out.push(cur);
      cur = w;
...
export const etagOf = (version, record) => `"${version}-${Date.parse(String(record.updated ?? "")) || 0}"`;
/** whether If-None-Match names this ETag (weak or strong) or everything */
export function notModified(ifNoneMatch, etag) {
  if (!ifNoneMatch)
    return false;
