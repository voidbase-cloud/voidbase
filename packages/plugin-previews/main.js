...
export { branchHash, branchSlug, PREVIEW_OF_VAR, PREVIEW_VAR, previewPrefix, previewWorkerName } from "@voidbase-cloud/voidbase/plugins/previews-names";
import { PREVIEW_OF_VAR, PREVIEW_VAR } from "@voidbase-cloud/voidbase/plugins/previews-names";
/**
 ...
 */
export const previewsInfo = (env) => {
  const read = (k) => String(env?.[k] ?? voidEnv[k] ?? "").trim();
  const of = read(PREVIEW_OF_VAR), branch = read(PREVIEW_VAR);
  if (of || branch)
    return { shape: "instance", ...(of ? { of } : {}), ...(branch ? { branch } : {}) };
...
export async function flaggedBranches(db, collections) {
  const byBranch = new Map();
  for (const c of flaggedCollections(collections)) {
    const rows = await all(db, `SELECT ${ident(PREVIEW_FIELD)} AS branch, COUNT(*) AS n FROM ${ident(c.name)} WHERE ${ident(PREVIEW_FIELD)} != '' GROUP BY ${ident(PREVIEW_FIELD)}`);
    for (const r of rows) {
      const entry = byBranch.get(String(r.branch)) ?? { branch: String(r.branch), rows: 0, collections: [] };
      entry.rows += Number(r.n);
      entry.collections.push(c.name);
      byBranch.set(entry.branch, entry);
    }
  }
  for (const e of byBranch.values())
    e.collections.sort();
...
export async function previewsReport(env) {
  const info = previewsInfo(env);
  if (info.shape !== "flagged")
    return info;
  try {
    return { ...info, branches: await flaggedBranches(env.DB, await listCollections(env.DB)) };
  }
  catch {
...
export async function removeFlagged(env, branch) {
  const deleted = {};
  let total = 0;
  for (const c of flaggedCollections(await listCollections(env.DB))) {
    const ids = await all(env.DB, `SELECT id FROM ${ident(c.name)} WHERE ${ident(PREVIEW_FIELD)} = ?`, [branch]);
    if (!ids.length)
      continue;
    await stmt(env.DB, `DELETE FROM ${ident(c.name)} WHERE ${ident(PREVIEW_FIELD)} = ?`, [branch]).run();
    deleted[c.name] = ids.length;
    total += ids.length;
    for (const r of ids) {
      try {
        await deleteAllRecordFiles(env.STORAGE, c.id, String(r.id));
      }
      catch { /* the rows are gone either way */ }
    }
  }
...
const branchParam = (c) => {
  ...
  if (!/^[A-Za-z0-9][\w./-]{0,99}$/.test(raw))
    throw badRequest(`branch is required: DELETE /api/previews?branch=<branch> (the value of ${PREVIEW_HEADER} the preview writes with).`);
  return raw;
};
