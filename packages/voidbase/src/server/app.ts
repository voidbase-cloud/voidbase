...
import { createKernel, load, loadedPlugin, runAfterRead, runBootstraps, using, whatLoaded } from "./kernel";
...
app.patch("/api/plugins/config/:name", async (c) => {
  requireSuperuser(c);
  return c.json(await setPluginConfig(c.env.DB, c.req.param("name"), await c.req.json().catch(() => null)));
});
